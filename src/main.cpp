...
// Copyright (c) 2017 DRGNcoin Developers
...
uint256 hashGenesisBlock("0x96893c7178dddf543297f6305c3398dfb10f386b2bf67606f8f5c2d2a8a7446e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DRGNcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "DRGNcoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if (nHeight >= 50000) {
        ...
        nSubsidy >>= (nHeight / 100000); // DRGNcoin: 100K blocks
    }

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // DRGNcoin: every 1 day
static const int64 nTargetSpacing = 120; // DRGNcoin: 120 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // DRGNcoin: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal DRGNcoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "DRGNcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x96893c7178dddf543297f6305c3398dfb10f386b2bf67606f8f5c2d2a8a7446e");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "DrgnCoin Pe7rieV9ItuoTohgh1ohBoo6iMiiZeix6ohmoodu2ePhoolaiGh2dohchielaegheez7shazewo0HiegaRaezaes8ohquaiz8wool7ooGhichoochahGhahn0eek7na1";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xd9a1f0df7abd47140c1af063b029d294b56965989192d0ca63562ea7ada08daa"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // DRGNcoin: increase each by adding 2 to bitcoin's value.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // Disconnect if not /DRGN:
        if (pfrom->strSubVer.empty() || pfrom->strSubVer.find("/DRGN:") == string::npos)
        {
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // DRGNcoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DRGNcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("DRGNcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadDRGNcoinMiner(void* parg);

static bool fGenerateBitcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void static DRGNcoinMiner(CWallet *pwallet)
{
    printf("DRGNcoinMiner started\n");
    ...
    while (fGenerateBitcoins)
    {
        ...
        printf("Running DRGNcoinMiner with %d transactions in block\n", pblock->vtx.size());
...
void static ThreadDRGNcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        DRGNcoinMiner(pwallet);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadDRGNcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadDRGNcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadDRGNcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d DRGNcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadDRGNcoinMiner, pwallet))
